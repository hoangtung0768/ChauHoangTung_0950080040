...
EXEC sp_InsertHangsx 'HSX01', 'Samsung', 'Hàn Quốc', '0912345678', 'contact@samsung.com'
...
BEGIN
    ...
    BEGIN
        ...
        WHERE sohdx = @sohdx
        PRINT 'Cập nhập dữ liệu thành công.'
    END
    ELSE
    BEGIN
        INSERT INTO Xuat(sohdx, masp, manv, ngayxuat, soluongX)
...
BEGIN
    ...
    PRINT 'xóa nhân viên với mã ' + CAST(@manv AS NVARCHAR)
END
...
BEGIN
  ...
  BEGIN TRY
    ...
    COMMIT TRANSACTION
    PRINT 'Xóa thành công' + @masp
  END TRY
  BEGIN CATCH
    ROLLBACK TRANSACTION
    PRINT 'Xảy ra lỗi trong quá trình xóa sản phẩm!'
  END CATCH
END